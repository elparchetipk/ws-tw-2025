...
document.addEventListener('click', (event) => {
    ...
    if (target.closest('.producto')) {
        ...
        if (!target.classList.contains('btn-comprar') && !target.classList.contains('btn-eliminar')) {
...
document.addEventListener('input', (event) => {
    ...
    if (target.id === 'precio') {
        ...
        const mensaje = document.querySelector('#mensaje-precio') || document.createElement('div');
...
class GestorTareasDOM {
    ...
    renderizarTareas() {
        ...
        container.innerHTML = tareasFiltradas.map(tarea => `
            <div class="tarea ${tarea.completada ? 'completada' : ''} prioridad-${tarea.prioridad}" 
                 data-id="${tarea.id}">
    ...
    actualizarEstadisticas() {
        ...
        document.getElementById('contador-pendientes').textContent = `${pendientes} pendientes`;
