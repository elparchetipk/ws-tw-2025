...
class RelojDigital {
    ...
    cargarConfiguraciones() {
        ...
        const configuracionesGuardadas = localStorage.getItem('reloj-configuraciones');
        return configuracionesGuardadas ? 
            { ...configuracionesPredeterminadas, ...JSON.parse(configuracionesGuardadas) } :
            configuracionesPredeterminadas;
    }
    
    guardarConfiguraciones() {
        localStorage.setItem('reloj-configuraciones', JSON.stringify(this.configuraciones));
    }
    ...
    aplicarConfiguraciones() {
        ...
        document.getElementById('sound-enabled').checked = this.configuraciones.sonidoActivado;
        document.getElementById('notifications-enabled').checked = this.configuraciones.notificacionesActivadas;
    }
    ...
    iniciarReloj() {
        ...
        this.establecerIntervalo('reloj-principal', () => this.actualizarReloj(), 1000);
    }
    ...
    actualizarReloj() {
        ...
        const horas = ahora.toLocaleString('es-ES', { ...opciones, hour: '2-digit' });
        const minutos = ahora.toLocaleString('es-ES', { ...opciones, minute: '2-digit' });
        const segundos = ahora.toLocaleString('es-ES', { ...opciones, second: '2-digit' });
        ...
        // Fecha
        const nombreDia = ahora.toLocaleDateString('es-ES', { weekday: 'long', timeZone: this.zonaHoraria });
        ...
        document.getElementById('day-name').textContent = nombreDia.charAt(0).toUpperCase() + nombreDia.slice(1);
    ...
    iniciarCronometro() {
        if (!this.cronometro.corriendo) {
            ...
            this.establecerIntervalo('cronometro', () => this.actualizarDisplayCronometro(), 10);
    ...
    registrarVuelta() {
        if (this.cronometro.corriendo) {
            ...
            const tiempoVuelta = this.cronometro.vueltas.length > 0 ? 
                tiempoActual - this.cronometro.vueltas[this.cronometro.vueltas.length - 1].tiempoTotal :
                tiempoActual;
    ...
    actualizarDisplayCronometro() {
        ...
        document.getElementById('stopwatch-minutes').textContent = minutos.toString().padStart(2, '0');
        document.getElementById('stopwatch-seconds').textContent = segundos.toString().padStart(2, '0');
        document.getElementById('stopwatch-milliseconds').textContent = milisegundos.toString().padStart(2, '0');
    }
    ...
    configurarTemporizador() {
        ...
        const minutos = parseInt(document.getElementById('timer-minutes').value) || 0;
        const segundos = parseInt(document.getElementById('timer-seconds').value) || 0;
    ...
    iniciarTemporizador() {
        if (!this.temporizador.corriendo && this.temporizador.tiempoRestante > 0) {
            ...
            this.establecerIntervalo('temporizador', () => this.actualizarTemporizador(), 100);
    ...
    actualizarTemporizador() {
        ...
        this.temporizador.tiempoRestante = Math.max(0, this.temporizador.duracionTotal - tiempoTranscurrido);
    ...
    actualizarDisplayTemporizador() {
        ...
        document.getElementById('timer-display-hours').textContent = horas.toString().padStart(2, '0');
        document.getElementById('timer-display-minutes').textContent = minutos.toString().padStart(2, '0');
        document.getElementById('timer-display-seconds').textContent = segundos.toString().padStart(2, '0');
        
        // Actualizar barra de progreso
        const progreso = ((this.temporizador.duracionTotal - tiempoRestante) / this.temporizador.duracionTotal) * 100;
    ...
    agregarAlarma() {
        ...
        const diasSeleccionados = Array.from(document.querySelectorAll('.alarm-days input:checked'))
        ...
        document.querySelectorAll('.alarm-days input').forEach(checkbox => checkbox.checked = false);
    }
    ...
    renderizarAlarmas() {
        ...
        if (this.alarmas.length === 0) {
            contenedor.innerHTML = '<p style="text-align: center; color: var(--text-secondary);">No hay alarmas configuradas</p>';
            return;
        }
            ...
            elemento.innerHTML = `
                <div class="alarm-info">
                    ...
                    <div class="alarm-days-display">
                        ${alarma.repetir ? this.formatearDias(alarma.dias) : 'Una vez'}
                    </div>
                </div>
                <div class="alarm-actions">
                    <button class="alarm-toggle ${alarma.activa ? 'active' : ''}" 
                            onclick="reloj.toggleAlarma(${alarma.id})"></button>
                    <button class="btn btn-danger btn-sm" 
                            onclick="reloj.eliminarAlarma(${alarma.id})">🗑️</button>
                </div>
            `;
    ...
    verificarAlarmas() {
        this.establecerIntervalo('verificar-alarmas', () => {
            const ahora = new Date();
    ...
    formatearTiempo(milisegundos) {
        ...
        return `${minutos.toString().padStart(2, '0')}:${segundos.toString().padStart(2, '0')}.${ms.toString().padStart(2, '0')}`;
    }
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (datos.configuraciones) {
                            this.configuraciones = { ...this.configuraciones, ...datos.configuraciones };
                            this.guardarConfiguraciones();
                            this.aplicarConfiguraciones();
                        }
    ...
    configurarEventos() {
        ...
        document.getElementById('stopwatch-start').addEventListener('click', () => this.iniciarCronometro());
        document.getElementById('stopwatch-pause').addEventListener('click', () => this.pausarCronometro());
        document.getElementById('stopwatch-reset').addEventListener('click', () => this.reiniciarCronometro());
        document.getElementById('stopwatch-lap').addEventListener('click', () => this.registrarVuelta());
        ...
        document.getElementById('timer-pause').addEventListener('click', () => this.pausarTemporizador());
        document.getElementById('timer-reset').addEventListener('click', () => this.reiniciarTemporizador());
        ...
        // Eventos de alarmas
        document.getElementById('add-alarm').addEventListener('click', () => this.agregarAlarma());
        ...
        document.getElementById('export-settings').addEventListener('click', () => this.exportarConfiguraciones());
        document.getElementById('import-settings').addEventListener('click', () => this.importarConfiguraciones());
        document.getElementById('reset-settings').addEventListener('click', () => this.resetearConfiguraciones());
        
        // Eventos de modal de alarma
        document.getElementById('dismiss-alarm').addEventListener('click', () => this.cerrarModalAlarma());
        document.getElementById('snooze-alarm').addEventListener('click', () => this.repetirAlarma());
